# OpsPilot 企业 IT 智能诊断平台 — Docker 多阶段构建
...
# 复制预下载的 ONNX 模型缓存
COPY --from=dependencies --chown=opspilot:opspilot /root/.cache/chroma /home/opspilot/.cache/chroma

# 复制应用代码
COPY --chown=opspilot:opspilot . .

# 创建必要目录，只调整运行期需要写入的目录权限，避免递归 chown 整个应用。
RUN mkdir -p /app/data/chroma /app/data/eval && \
...
RUN mkdir -p /app/data/chroma /app/data/eval /app/tests && \
